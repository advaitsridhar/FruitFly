...
| endpoint | returns |
|---|---|
| `GET /api/types?q=LC10[&limit=50]` | `{types: [{type, n[, alias_of]}]}` (a name the fly answers to as an alias, such as the female fly's `MN9` for FlyWire's `CB0701`, is listed too, with `alias_of` giving what it stands for) |
